...
async function applyMigration() {
  try {
    ...
    const migrationPath = path.join(__dirname, 'migrations', 'user_settings.sql');
    const migrationSql = fs.readFileSync(migrationPath, 'utf8');
    
    // Execute the SQL statement using the Supabase REST API
    // Note: This requires the pg_admin role to execute SQL directly
    // Alternative: You need to go to the Supabase dashboard and run the SQL in the SQL editor
